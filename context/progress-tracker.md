...
- **07 AI Profile Extraction** — Extract from Resume (after upload) POSTs the PDF to `POST /api/resume/extract` with InsForge Bearer JWT (`lib/api-auth.ts`). Server uses `pdf-parse` v2 (`PDFParse`) then Vercel AI SDK `generateObject` via OpenRouter model `openrouter/free` (response-healing). Env: server-only `OPENROUTER_API_KEYS` (multi-key failover) or `OPENROUTER_API_KEY`, plus `AI_PROVIDER`, `AI_MODEL`. Client merges into form state via `mergeExtractedIntoProfile` (no auto-save). Provider swap + key rotation live in `lib/ai/provider.ts`.
