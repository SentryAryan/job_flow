...
- [x] 17 Analytics Charts — InsForge inventory + PostHog Product Insights
...
- **05 Profile Page — UI primitives** — Replaced with **shadcn/ui** (`components.json`, radix-nova). Job Flow tokens remain source of truth in `app/globals.css` (`--jp-*` + `@theme`); shadcn semantic vars map onto them. Prefer shadcn primitives for all form controls (Select / Checkbox / Button) — no native `<select>` wrappers.
...
- **UI — shadcn/ui** — Initialized shadcn (`radix` + `nova`, `components.json`). Primitives under `components/ui/` from the registry; Job Flow colors kept via `--jp-*` / `@theme` (brand purple stays `bg-accent`). Custom toaster (`toaster.tsx`) kept over default sonner for token-styled toasts. **Prefer shadcn for everything** — use registry `Select` / `Checkbox` / `Button` instead of native `<select>` / checkbox / raw buttons when counterparts exist. Removed `NativeSelect`. Progress used for usage card.
...
- **17 Analytics Charts — InsForge + Product Insights** — Inventory charts from InsForge (`GET /api/dashboard/charts/jobs-over-time|match-distribution|research-activity?range=`): shared range `7d|30d|60d|all` (shadcn Select). Jobs over time / match buckets use `found_at` + `match_score`; research activity uses `researched_at`. Fixed ranges zero-fill days; `all` is sparse. **Product Insights** (`GET /api/dashboard/insights`): PostHog `job_search_started` series + feature usage bars (`resume_generated`, `company_researched`, `profile_completed`); missing Query credentials → empty series (200). Helpers in `lib/dashboard-charts.ts`, `lib/dashboard-range.ts`, `lib/dashboard-insights.ts`. Docker Compose (`docker-compose.yml`) for local/prod-parity; Render still uses Dockerfile + `render.yaml`.
- **Theme switcher (app-wide)** — `next-themes` with `class` on `<html>`, `defaultTheme=system`, `storageKey=jobflow-theme`. Dark palette redefines `--jp-*` under `.dark` (brand purple kept). `ThemeSwitcher` in shared Navbar (Light / Dark / System). CTA hover uses `bg-cta-hover` token. Sonner toaster follows `resolvedTheme`. Future pages inherit via tokens; import `ThemeSwitcher` only if a page lacks Navbar.
