...
- **CTA routing** — Marketing “Get Started” / “Find Your First Match” go to `/login` when signed out and `/find-jobs` when signed in (homepage stays reachable; no bounce to dashboard via logo). Navbar CTA is **Profile** → `/profile`.
