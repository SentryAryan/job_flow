...
- Profile form chrome: skills/industry chips use purple/accent styling; "Extract from Resume" must match "Save Profile" button styling; "Clear all fields" is a red destructive control with a full solid-color hover fill (never black or partial coverage) and clears every field except the uploaded resume.
...
- Feature 05/06 Profile: UI from `context/designs/profile.png` under `components/profile/` (AuthGuard-wrapped `/profile`); loads/saves via `lib/profile.ts` (browser InsForge client); `is_complete` from `getProfileCompletion`; resume PDF ≤5MB to `resumes` with upload-first then cleanup of prior keys (never delete-before-upload); private-bucket View/Download use signed/download URLs for PDF preview (modal/new tab) and `.pdf` download — not raw private object URLs; InsForge may auto-rename on key collision — treat storage `remove` "Object not found" as benign; PostHog `profile_completed` on incomplete→complete; save feedback via Sonner toasts. Cover letter tone omitted from UI. Generate Resume stays Feature 08 stub.
- Feature 07 AI Profile Extraction: **Extract from Resume** on `/profile` after upload → client Bearer JWT + PDF → `POST /api/resume/extract` (`lib/api-auth.ts` + `pdf-parse` + AI SDK `generateObject`). Model via `lib/ai/provider.ts`: OpenRouter `openrouter/free` only; env `OPENROUTER_API_KEYS` (comma-separated, whitespace around commas trimmed) or `OPENROUTER_API_KEY`, plus `AI_PROVIDER`, `AI_MODEL`. Rate-limit/quota errors rotate keys in-request via `withOpenRouterKeyFailover`. Production extract abuse limits use Redis sliding windows (`REDIS_URL`, `APP_ENV=production|prod`; skipped in `development`/`dev`). Extraction must recover LinkedIn/portfolio/GitHub from PDF hyperlink annotations (not only visible text), fill sparse model output with resume-based fallbacks, reject salary-only “success”, and merge into form state (`mergeExtractedIntoProfile`) — user reviews then saves manually. Next up: Feature 08 Resume PDF Generation.
- App unit/integration tests use Vitest (`vitest.config.ts`; `npm test` / `npx vitest run`).
