...
- Feature 07 AI Profile Extraction: **Extract from Resume** on `/profile` after upload → client Bearer JWT + PDF → `POST /api/resume/extract` (`lib/api-auth.ts` + `pdf-parse` + AI SDK `generateObject`). Model via `lib/ai/provider.ts`: OpenRouter `openrouter/free` only; env `OPENROUTER_API_KEYS` (comma-separated, whitespace around commas trimmed) or `OPENROUTER_API_KEY`, plus `AI_PROVIDER`, `AI_MODEL`. Rate-limit/quota errors rotate keys in-request via `withOpenRouterKeyFailover`. Extraction must recover LinkedIn/portfolio/GitHub from PDF hyperlink annotations (not only visible text), fill sparse model output with resume-based fallbacks, and merge into form state (`mergeExtractedIntoProfile`) — user reviews then saves manually. Next up: Feature 08 Resume PDF Generation.
