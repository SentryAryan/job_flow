...
- Feature 07 AI Profile Extraction: **Extract from Resume** on `/profile` after upload → client Bearer JWT + PDF → `POST /api/resume/extract` (`lib/api-auth.ts` + `pdf-parse` + AI SDK `generateObject`). Model via `lib/ai/provider.ts`: OpenRouter `openrouter/free` only; env `OPENROUTER_API_KEYS` (comma-separated failover) or `OPENROUTER_API_KEY`, plus `AI_PROVIDER`, `AI_MODEL`. Rate-limit/quota errors rotate keys in-request via `withOpenRouterKeyFailover`. Merges into form state (`mergeExtractedIntoProfile`) — user reviews then saves manually. Next up: Feature 08 Resume PDF Generation.
